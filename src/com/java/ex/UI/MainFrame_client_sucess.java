...
public class MainFrame_client_sucess extends JFrame {
	...
	public MainFrame_client_sucess(String id) {
		getContentPane().setLayout(null);
		tp = new JTabbedPane();
		tp.setBounds(0, 61, 784, 385); 
		returns = new return_page(id);
		reStatus = new rental_status_page(id);
		exit = new exit_page();
		rental = new rental_page(id);
		...
		loginBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// 로그아웃 버튼 
				MainFrame_client  mc = new MainFrame_client();
				mc.setVisible(true);
				dispose();
		...
		public static void main(String[] args) {
			new MainFrame_client_sucess(null); 
	}
}