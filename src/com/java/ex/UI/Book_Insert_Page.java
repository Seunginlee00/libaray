...
public class Book_Insert_Page extends JFrame{
	...
	public Book_Insert_Page() {
		...
		JLabel BookNumberLabel = new JLabel("번호");
		BookNumberLabel.setFont(new Font("굴림", Font.PLAIN, 16));
		BookNumberLabel.setBounds(32, 90, 55, 25);
		...
		BookNumberField.setFont(new Font("굴림", Font.PLAIN, 16));
		BookNumberField.setBounds(115, 90, 182, 25);
		...
		saveBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					bDto = new BookDto(generalBox.getSelectedItem().toString(), Integer.parseInt(BookNumberField.getText()), BookNameField.getText(), BookAuthourField.getText(), publisherField.getText(), 0, "Y", "N");	
				
					bDao = new BookDao();
					bDao.getBookInsert(bDto);
				...
				if (bDao != null) {
					JOptionPane.showConfirmDialog(BookNameField, BookNameField.getText() + "도서 추가!");
				}
			}
		});
...
public static void main(String[] args) {
	EventQueue.invokeLater(new Runnable() {
		public void run() {
			try {
				Book_Insert_Page window = new Book_Insert_Page();
